...
// console.log("validator",validator);
    authRouter.post("/signup", async (req,res)=>{
      ...
      try{
          const {fullName,password,gmail,}=req.body;
        // validateData(req); //this function is used to validate the data
        
        const loginUser=  await User.findOne({gmail:gmail}); // await laga na jaruri he kyo ki find wala 
        ...
        await user.save();
        res.status(200).json({message:"data inserted",data:user});
      }catch(err)
      {
    ...
    authRouter.post("/login", async(req,res)=>
    {
      ...
      try{
        ...
        if(!user)
        {
          throw new Error("invalid credentials gmail ");
        }
        else
        {
